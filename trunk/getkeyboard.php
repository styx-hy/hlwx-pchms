 ...
 * ========================
 */
$query = "SELECT * FROM (SELECT span*60000*$_GET[interval] as stamp, times FROM (SELECT CEIL(UNIX_TIMESTAMP(dtstamp)/60.0/$_GET[interval]) span, COUNT(dtstamp) times FROM keytable WHERE userid=('$_SESSION[userid]') GROUP BY span) x ) y WHERE stamp<=UNIX_TIMESTAMP(date_add(curdate(), interval -'$_GET[time_span_s]' day))*1000 and stamp >= UNIX_TIMESTAMP(date_add(curdate(),interval -'$_GET[time_span_e]' day))*1000;";
...
$query = "SELECT * FROM (SELECT span*60000*$_GET[interval] as stamp, times FROM (SELECT CEIL(UNIX_TIMESTAMP(dtstamp)/60.0/$_GET[interval]) span, COUNT(dtstamp) times FROM keytable GROUP BY span) x ) y WHERE stamp<=UNIX_TIMESTAMP(date_add(curdate(), interval -'$_GET[time_span_s]' day))*1000 and stamp >= UNIX_TIMESTAMP(date_add(curdate(),interval -'$_GET[time_span_e]' day))*1000;";
$result = mysql_query($query, $con);
